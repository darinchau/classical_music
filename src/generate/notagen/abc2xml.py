...
def lmap(f, xs): return list(map(f, xs))   # eager map for python 3
        ...
        if x.name == 'chord':
            ...
            for nt in nss:   # all chord elements (note | decorations | rest | grace note)
                if nt.name == 'note':
                    ...
                    if j > 0:
                        nt.chord = pObj('chord', [1])  # label all but first as chord notes
...
def splitHeaderVoices(abctext):
    def escField(x): return '[' + x.replace(']', r'%5d') + ']'  # hope nobody uses %5d in a field
    ...
    if len(xs) == 1:
        ...
        if pm:
            xs[2] = ''.join(pm) + xs[2]         # prepend P:-marks to the text of the first voice
    ...
    for x in p.objs:
        if type(x) == pObj:
            if x.name == 'vid':
                ...
                xs.extend(mkGroups(x) + ['}'])  # x.name == 'grand' == rejected grand staff
        else:
            xs.append(p.t[0])
    return xs
...
class MusicXml:
    ...
    def mkNote(s, n, lev):
        ...
        if (s.midprg != ['', '', '', ''] or midi) and n.name != 'rest':  # only add when %%midi was present or percussion
            ...
            if instId not in s.midiInst:
                s.midiInst[instId] = (s.pid, s.vid, chan, midi, s.midprg[2], s.midprg[3])  # for instrument list in mkScorePart
        addElemT(nt, 'voice', '1', lev + 1)    # default voice, for merging later
        if noMsrRest:
            addElemT(nt, 'type', xmltype, lev + 1)  # add note type if not a measure rest
        ...
        if s.ntup >= 0:                 # add time modification element for tuplet notes
            ...
            if s.ntup == 0:             # last tuplet note (and possible chord notes there after)
                if rdvs:
                    tupnotation = 'stop'   # only insert notation in the real note (rdvs > 0)
    ...
    def doFields(s, maat, fieldmap, lev):
        ...
        for ftype, field in fieldmap.items():
            ...
            if ftype == 'Div':  # not an abc field, but handled as if
                ...
                try:
                    s.mdur = simplify(eval(beats), int(btype))  # measure duration for Z and X rests (eval allows M:2+3/4)
                except:
    ...
    def doTempo(s, maat, field, lev):
        ...
        if t:
            try:
                if t.group(4):
                    num, den, upm = 1, s.unitLcur[1], float(t.group(4))  # old syntax Q:120
                else:
    ...
    def mkScorePart(s, id, vids_p, partAttr, lev):
        def mkInst(instId, vid, midchan, midprog, midnot, vol, pan, lev):
            ...
            if midprog:
                addElemT(mi, 'midi-program', str(int(midprog) + 1), lev + 2)  # compatible with abc2midi
            if midnot:
                addElemT(mi, 'midi-unpitched', str(int(midnot) + 1), lev + 2)
            if vol:
                addElemT(mi, 'volume', '%.2f' % (int(vol) / 1.27), lev + 2)
            if pan:
                addElemT(mi, 'pan', '%.2f' % (int(pan) / 127. * 180 - 90), lev + 2)
            return (si, mi)
    ...
    def doField_I(s, type, x, instDir, addTrans):
        ...
        def readPercMap(x):    # parse I:percmap <abc_note> <step> <MIDI> <notehead>
            def getMidNum(sndnm):          # find midi number of GM drum sound name
                ...
                def _f(ip, xs, pnm): return ip < len(xs) and xs[ip].find(pnm) > -1   # part xs[ip] and pnm match
        ...
        if x.startswith('score') or x.startswith('staves'):
            ...
            if r1:
                ...
                if sign:                                # relative staff number
                    ...
                    try:
                        vabc = s.staves[num - 1][0]   # vid of (first voice of) abc-staff num
                    except:
                        vabc = 0
    ...
    def parseStaveDef(s, vdefs):
        ...
        def f(x): return type(x) == uni_type and [x] or x
        ...
        try:
            ...
            for id, voice in voices:
                if lbrk_insert:                                 # insert linebreak at EOL
                    ...
                    def has_abc(x): return r1.sub('', x).strip()  # empty if line only contains inline fields
...
def fixDoctype(elem):
    if python3:
        xs = E.tostring(elem, encoding='unicode')  # writing to file will auto-encode to utf-8
    else:
        xs = E.tostring(elem, encoding='utf-8')    # keep the string utf-8 encoded for writing to file
    ys = xs.split('\n')
...
def convert_abc_to_xml(input_files, output_dir='', skip=0, num=1, page_format='', mxl_mode='', show_whole_measure_rests=False, use_title_as_filename=False, line_break_at_eol=False, meta_map='', force_string_fret_allocations=False):
    """
    Convert ABC files to MusicXML format.

    Args:
        input_files (list): List of input ABC files.
        output_dir (str): Directory to store XML files. Default is ''.
        skip (int): Number of tunes to skip. Default is 0.
        num (int): Number of tunes to read. Default is 1.
        page_format (str): Page format in mm. Default is ''.
        mxl_mode (str): Store as compressed MXL, mode can be 'a(dd)' or 'r(eplace)'. Default is ''.
        show_whole_measure_rests (bool): Show whole measure rests in merged staffs. Default is False.
        use_title_as_filename (bool): Use tune title as file name. Default is False.
        line_break_at_eol (bool): Line break at EOL. Default is False.
        meta_map (str): Map infofields to XML metadata. Default is ''.
        force_string_fret_allocations (bool): Force string/fret allocations for tab staves. Default is False.
    """
    if output_dir:
    ...
    if mxl_mode and mxl_mode not in ['a', 'add', 'r', 'replace']:
        raise ValueError(f'MODE should be a(dd) or r(eplace), not: {mxl_mode}')

    if page_format:   # set page formatting values
        try:        # space, page-height, -width, margin-left, -right, -top, -bottom
            mxm.pageFmtCmd = lmap(float, page_format.split(','))
            if len(mxm.pageFmtCmd) != 7:
                raise ValueError('-p needs 7 values')
        except Exception as err:
            raise ValueError(err)

    for x in meta_map.split(','):
        if not x:
            continue
        try:
            field, tag = x.split(':')
        except:
            raise ValueError(f'--meta: {x} cannot be split on colon')
        if field not in 'OAZNGHRBDFSPW':
            raise ValueError(f'--meta: field {field} is no valid ABC field')
        if tag not in mxm.metaTypes:
            raise ValueError(f'--meta: tag {tag} is no valid XML creator type')
    ...
    for i in input_files:
        if i == '-':
            ...
            fnmext_list += glob(i)
    if not fnmext_list:
        raise ValueError('none of the input files exist')

    t_start = time.time()
    for fnmext in fnmext_list:
        fnm, ext = os.path.splitext(fnmext)
        if ext.lower() not in ('.abc'):
            info(f'skipped input file {fnmext}, it should have extension .abc')
            continue
        if os.path.isdir(fnmext):
            info(f'skipped directory {fnmext}. Only files are accepted')
            continue
        abctext = readfile(fnmext)
        xml_docs = getXmlDocs(abctext, skip, num, show_whole_measure_rests, line_break_at_eol, force_string_fret_allocations)
        for itune, xmldoc in enumerate(xml_docs):
            fnmNum = f'{itune + 1:02d}' if len(xml_docs) > 1 else ''
            writefile(output_dir, fnm, fnmNum, xmldoc, mxl_mode, use_title_as_filename)
    info(f'done in {time.time() - t_start:.2f} secs')
